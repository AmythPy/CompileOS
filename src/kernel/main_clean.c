/**
 * CompileOS - CLEAN MAIN KERNEL
 * Desktop-integrated version without conflicts
 */
...
// Include desktop header for window_t type
...
void kernel_main(void) {
    ...
    if (desktop_init() != 0) {
        // Fallback to simple VGA if desktop fails
        vga_clear_multi(0x07);
        vga_puts_multi("Desktop initialization failed - using simple terminal", 0, 0, 0x0C);
        vga_puts_multi("CompileOS v0.1.0 - Interactive Terminal", 0, 2, 0x0F);
        vga_puts_multi("Type 'help' for commands", 0, 4, 0x0B);
    } else {
        // Start desktop
        if (desktop_start() != 0) {
            vga_puts_multi("Desktop start failed", 0, 6, 0x0C);
        } else {
            vga_puts_multi("Desktop started successfully", 0, 6, 0x0A);
        }
    }

    // Create main terminal window
    window_t* terminal_window = desktop_create_window("CompileOS Terminal", 1, 1, 78, 21);
    if (terminal_window) {
        vga_puts_multi("Terminal window created", 0, 8, 0x0A);
    }
